		...
		<div style = "padding : 20px">
			...
			<div align = "Center" id = "eventList">
	 			<table class = "table table-striped table-responsive table-hover table-striped">
	 				...
	 				<td> <%=notification.getStaffID() %> </td>
	 				<td> <%=notification.getEname() %> </td>
	 				<td> <%=notification.getMassages() %> </td>
	 				<td>
	 					<form method="post" action="../GetNotificationServlet">
	 						<input type="hidden" name="NID" value = "<%= notification.getNID() %>"></input>
	 						<button type = "submit" class = "btn btn-outline-info waves-effect px-3"><i class = "fas fa-edit" aread-hidden="false"></i></button>
	 					</form>
	 				</td>
	 				
	 				<td>
	 					<form method="post" action="../DeleteNotificationServlet">
	 						<input type="hidden" name="NID" value = "<%= notification.getNID() %>"></input>
				...
				<div>
						<a id="singlebutton" href="AddNotification.jsp"
						class="btn btn-primary">ADD MASSAGE</a>
				</div>
	</div></div></div></div>
